...
if (process.env.FIREBASE_PRIVATE_KEY) {
	...
	console.log(
		"   Has BEGIN marker:",
		key.includes("BEGIN PRIVATE KEY") ? "✅ Yes" : "❌ No",
	);
	console.log(
		"   Has END marker:",
		key.includes("END PRIVATE KEY") ? "✅ Yes" : "❌ No",
	);
	...
	if (key.includes("\\n") && !key.includes("\n")) {
		console.log("\n❌ ISSUE DETECTED:");
		console.log(
			'   Your .env file has literal "\\n" instead of actual newlines.',
		);
		console.log(
			"   The dotenv library will convert \\n to newlines automatically.",
		);
		console.log(
			"   Make sure you are using DOUBLE quotes around the key value.",
		);
	}
