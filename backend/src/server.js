...
function startServer() {
	...
	try {
		...
		const startupTimeout = setTimeout(() => {
			console.error(
				"❌ Server startup timeout - failed to bind to port within 30 seconds",
			);
		...
		server.listen(PORT, HOST, () => {
			...
			const hostUrl = IS_PRODUCTION
				? `https://${process.env.CALL_SIGN || "groundctrl"}.web.app`
				: `http://localhost:${PORT}`;
		...
		server.on("error", (error) => {
			...
			if (error.code === "EADDRINUSE") {
				...
				console.error(
					`   Unexpected server error: ${error.code || error.message}`,
				);
				process.exit(1);
			}
		});
