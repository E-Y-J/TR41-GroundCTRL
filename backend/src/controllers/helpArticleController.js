...
const crudHandlers = createCrudHandlers(
	...
	"help_article",
	{
		...
		update: {
			safeParse: (body) =>
				updateArticleSchema.safeParse({ body, params: { id: body.id } }),
		},
		patch: {
			safeParse: (body) =>
				patchArticleSchema.safeParse({ body, params: { id: body.id } }),
		},
	},
	{
		...
		beforeUpdate: async (req, updates) => {
			...
			if (updates.slug && updates.slug !== existing.slug) {
				const slugExists = await helpArticleRepository.existsBySlug(
					updates.slug,
					id,
				);
				if (slugExists) {
					throw new ConflictError(
						`Article with slug '${updates.slug}' already exists`,
					);
				}
			}
		},
...
async function getArticleBySlug(req, res, next) {
	try {
		...
		helpArticleRepository.incrementViews(article.id).catch((err) => {
			logger.error("Failed to increment article views", {
				id: article.id,
				error: err.message,
			});
		});
...
async function searchArticles(req, res, next) {
	try {
		...
		logger.info("Help articles searched", {
			searchTerm,
...
async function getPopularArticles(req, res, next) {
	try {
		...
		logger.info("Popular help articles retrieved", {
