...
const crudHandlers = createCrudHandlers(
	...
	"help_category",
	{
		...
		update: {
			safeParse: (body) =>
				updateCategorySchema.safeParse({ body, params: { id: body.id } }),
		},
		patch: {
			safeParse: (body) =>
				patchCategorySchema.safeParse({ body, params: { id: body.id } }),
		},
	},
	{
		...
		beforeUpdate: async (req, updates) => {
			...
			if (updates.code && updates.code !== existing.code) {
				const codeExists = await helpCategoryRepository.existsByCode(
					updates.code,
					id,
				);
				if (codeExists) {
					throw new ConflictError(
						`Category with code '${updates.code}' already exists`,
					);
				}
			}
		},
