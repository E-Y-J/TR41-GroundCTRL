...
async function execute(req, res, next) {
	try {
		...
		const { command_name, command_payload, session_id, scenario_step_id } =
			parseResult.data;
		...
		const executionResult = simulateCommandExecution(
			command_name,
			command_payload,
		);
...
async function getRegistry(req, res) {
	const commandRegistry = {
		...
		system: ["SYSTEM_RESET", "SAFE_MODE", "NOMINAL_MODE", "RUN_DIAGNOSTICS"],
	};
