...
async function getAll(options = {}) {
	try {
		...
		logger.error("Failed to fetch all scenario steps", {
			error: error.message,
		});
		throw error;
	}
...
async function create(data, metadata = {}) {
	try {
		...
		logger.info("Scenario step created", {
			id: docRef.id,
...
async function update(id, data, metadata = {}) {
	try {
		...
		logger.error("Failed to update scenario step", {
			error: error.message,
			id,
		});
		throw error;
...
async function deleteStep(id, metadata = {}) {
	try {
		...
		logger.error("Failed to delete scenario step", {
			error: error.message,
			id,
		});
		throw error;
