...
async function getAll(options = {}) {
  try {
    ...
    logger.error('Failed to fetch all scenario sessions', { error: error.message });
    throw error;
  }
}
...
async function create(data, metadata = {}) {
  try {
    ...
    logger.info('Scenario session created', { id: docRef.id, scenario_id: data.scenario_id });
...
async function update(id, data, metadata = {}) {
  try {
    ...
    logger.error('Failed to update scenario session', { error: error.message, id });
    throw error;
  }
}
...
async function patch(id, updates, metadata = {}, expectedVersion = null) {
  try {
    ...
    logger.error('Failed to patch scenario session', { error: error.message, id });
    throw error;
  }
}
...
async function deleteSession(id, metadata = {}) {
  try {
    ...
    logger.info('Scenario session deleted', { id, deletedBy: metadata.deletedBy });
  } catch (error) {
    logger.error('Failed to delete scenario session', { error: error.message, id });
    throw error;
  }
}
...
function sanitizeSession(session) {
  ...
  if (sanitized.startedAt?.toDate) sanitized.startedAt = sanitized.startedAt.toDate().toISOString();
  if (sanitized.completedAt?.toDate) sanitized.completedAt = sanitized.completedAt.toDate().toISOString();
  return sanitized;
}
