...
async function getBySessionId(sessionId, options = {}) {
  try {
    ...
    logger.error('Failed to fetch commands by session', { error: error.message, sessionId });
    throw error;
  }
}
...
async function getRecentByUser(userId, limit = 10) {
  try {
    ...
    logger.error('Failed to fetch recent commands for user', { error: error.message, userId });
    throw error;
  }
}
...
function sanitizeCommand(command) {
  ...
  if (sanitized.issuedAt?.toDate) sanitized.issuedAt = sanitized.issuedAt.toDate().toISOString();
