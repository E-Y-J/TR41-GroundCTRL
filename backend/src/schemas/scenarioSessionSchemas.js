...
const createScenarioSessionSchema = z
	.object({
		body: z
			.object({
				...
				status: SessionStatus.default("NOT_STARTED").describe(
...
const updateScenarioSessionSchema = z
	.object({
		params: z
			.object({
				id: z
...
const patchScenarioSessionSchema = z
	.object({
		params: z
			.object({
				id: z
...
const listScenarioSessionsSchema = z
	.object({
		query: z
			.object({
				page: z
...
const updateStateSchema = z
	.object({
		...
		body: z
			.object({
				state: z.object({}).passthrough().describe("Updated simulation state"),
				hints_used: z
					.number()
					.int()
					.min(0)
