...
class VisibilityCalculator {
	...
	calculateVisibility(satelliteOrbit, groundStation, currentTime) {
		...
		const localCoords = this.eciToLocal(
			satPosition,
		...
		const rangeRate = this.calculateRangeRate(
			satelliteOrbit,
			groundStation,
			currentTime,
		);
		...
		if (isVisible) {
			...
			const passInfo = this.calculatePassDuration(
				satelliteOrbit,
				groundStation,
				currentTime,
			);
			...
			// Calculate next pass
			nextPassTime = this.calculateNextPass(
				satelliteOrbit,
				groundStation,
				currentTime,
			);
	...
	eciToLocal(satPosition, gsLocation, time) {
		...
		const south =
			-Math.sin(lat) * Math.cos(lmst) * dx -
			Math.sin(lat) * Math.sin(lmst) * dy +
			Math.cos(lat) * dz;
		const east = -Math.sin(lmst) * dx + Math.cos(lmst) * dy;
		const zenith =
			Math.cos(lat) * Math.cos(lmst) * dx +
			Math.cos(lat) * Math.sin(lmst) * dy +
			Math.sin(lat) * dz;
	...
	calculateAzimuth(localCoords) {
		const azimuth =
			(Math.atan2(localCoords.east, -localCoords.south) * 180) / Math.PI;
	...
	calculateRangeRate(orbit, groundStation, currentTime) {
		...
		const pos2 = this.calculateVisibility(
			orbit,
			groundStation,
			currentTime + dt * 1000,
		);
