...
function getGeminiModel() {
	...
	if (!apiKey) {
		logger.warn(
			"GEMINI_API_KEY not configured - NOVA will use fallback responses",
		);
		return null;
	}
...
async function fetchContext(sessionId, stepId = null) {
	...
	try {
		...
		// Get recent commands
		const commands = await commandRepository.getBySessionId(sessionId, {
			limit: MAX_RECENT_COMMANDS,
		});
		context.recentCommands = commands;

		// Get conversation history
		const history = await aiMessagesRepository.getRecentMessages(
			sessionId,
			MAX_HISTORY_MESSAGES,
		);
		context.sessionHistory = history;
	} catch (error) {
		logger.error("Failed to fetch NOVA context", {
			error: error.message,
			sessionId,
		});
...
function getFallbackResponse(context, _userMessage) {
	...
	if (context.currentStep?.hint_suggestion) {
		...
		response =
			"NOVA here. I'm experiencing some communication delays. Please continue with your current task, and I'll be back with full assistance shortly. Review your command console for immediate feedback.";
	}
...
async function generateNovaResponse(
	...
	await aiMessagesRepository.addMessage(
		sessionId,
		userId,
		"user",
		userMessage,
...
async function storeResponse(sessionId, userId, content, metadata = {}) {
	return aiMessagesRepository.addMessage(
		sessionId,
		userId,
		"assistant",
		content,
...
async function incrementSessionHints(sessionId) {
	try {
		...
		logger.error("Failed to increment session hints", {
			error: error.message,
			sessionId,
		});
...
async function fetchUserHistory(userId) {
	try {
		...
		for (const session of sessions.data.slice(0, 5)) {
			if (session.scenario_id) {
				try {
					const scenario = await scenarioRepository.getById(
						session.scenario_id,
					);
 ...
 */
async function fetchHelpContext(
	userMessage,
	articleSlug = null,
	conversationId = null,
	userHistory = null,
	...
	try {
		...
		const searchResults = await helpArticleRepository.search(
			userMessage,
			{},
			3,
		);
		...
		const faqsResult = await helpFaqRepository.getAll({
			status: "PUBLISHED",
			isActive: true,
		});
		...
		logger.error("Failed to fetch help context", {
			error: error.message,
			userMessage,
		});
		// Continue with partial context
...
async function generateHelpResponse(userMessage, options = {}) {
	...
	if (!model) {
		...
		const fallbackContent =
			helpContext.helpArticles.length > 0
	...
	if (hasActiveSession) {
		...
		response = {
			...
			context: {
				...
				suggestion:
					"Sign in to unlock personalized training scenarios and track your progress!",
				auth_benefits: [
 ...
 * @returns {string} Formatted prompt
 */
function buildAuthenticatedHelpPrompt(
	context,
	userMessage,
	callSign,
	userProfile = null,
	...
	if (userProfile) {
		if (userProfile.displayName)
			contextSection += `Display Name: ${userProfile.displayName}\n`;
...
function splitIntoParagraphs(content) {
	...
	if (paragraphs.length === 1 && paragraphs[0].length > 400) {
		...
		for (const sentence of sentences) {
			if (
				currentChunk.length + sentence.length > 300 &&
...
function generateSuggestions(context, content) {
	...
	if (contentLower.includes("training") || contentLower.includes("module")) {
		...
		if (filtered.length > 0)
			return filtered
				.concat(contextSuggestions.filter((s) => s.id !== "modules"))
				.slice(0, 2);
	}
	...
	if (contentLower.includes("mission") || contentLower.includes("scenario")) {
		...
		if (filtered.length > 0)
			return filtered
				.concat(contextSuggestions.filter((s) => s.id !== "recommend"))
				.slice(0, 2);
	}
	...
	if (contentLower.includes("command") || contentLower.includes("execute")) {
		...
		if (filtered.length > 0)
			return filtered
				.concat(contextSuggestions.filter((s) => s.id !== "command"))
				.slice(0, 2);
	}
