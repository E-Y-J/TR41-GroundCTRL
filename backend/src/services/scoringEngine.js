...
class ScoringEngine {
		...
		this.achievements = [
			{
				...
				check: (session) => {
					const burns =
						session.commandHistory?.filter((c) => c.name === "EXECUTE_BURN") ||
						[];
	...
	calculateTiming(sessionData) {
		...
		const avgError =
			timingErrors.reduce(
				(sum, error) => sum + Math.abs(error.actual - error.optimal),
				0,
			) / timingErrors.length;

		// Calculate average acceptable window
		const avgWindow =
			timingErrors.reduce((sum, error) => sum + error.window, 0) /
			timingErrors.length;
