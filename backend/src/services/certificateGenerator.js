...
class CertificateGenerator {
		...
		const certificate = {
			...
			performance: {
				...
				duration: this.formatDuration(
					sessionMetrics.endTime - sessionMetrics.startTime,
				),
			...
			// Shareable data
			shareableText: this.generateShareableText(
				userName,
				scenario.name,
				sessionMetrics,
			),
	...
	analyzePerformance(metrics) {
		...
		if (metrics.scores.commandAccuracy >= 90) {
			strengths.push(
				"Excellent command accuracy - you issued the right commands at the right time",
			);
		...
		if (metrics.scores.responseTime >= 90) {
			strengths.push(
				"Optimal response time - balanced speed with careful decision-making",
			);
		...
		if (metrics.scores.resourceManagement >= 85) {
			strengths.push(
				"Excellent resource management - maintained power and fuel efficiently",
			);
		...
		if (metrics.errors.count === 0) {
			...
			improvements.push(
				"Avoid critical errors by double-checking commands before execution",
			);
		}
	...
	generateShareableText(userName, scenarioName, metrics) {
		...
		if (metrics.achievements.length > 0) {
			const achievementIcons = metrics.achievements
				.map((a) => a.badge)
				.join(" ");
