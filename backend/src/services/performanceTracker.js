...
class PerformanceTracker {
	constructor() {
		...
		// Scoring weights
		this.WEIGHTS = {
			COMMAND_ACCURACY: 0.3, // 30% - correct commands
			RESPONSE_TIME: 0.2, // 20% - timely responses
			RESOURCE_MANAGEMENT: 0.25, // 25% - power/fuel efficiency
			COMPLETION_TIME: 0.15, // 15% - mission duration
			ERROR_AVOIDANCE: 0.1, // 10% - avoiding errors
		};
	...
	recordResponseTime(sessionId, responseTime) {
		...
		metrics.timing.averageResponseTime =
			sum / metrics.timing.responseTimes.length;
	...
	recordError(sessionId, errorType, severity = "minor") {
		...
		metrics.errors.types[errorType] =
			(metrics.errors.types[errorType] || 0) + 1;
	...
	updateResources(sessionId, telemetry) {
		...
		metrics.resources.powerEfficiency =
			(metrics.resources.finalPower / metrics.resources.initialPower) * 100;
		metrics.resources.fuelEfficiency =
			(metrics.resources.finalFuel / metrics.resources.initialFuel) * 100;
	...
	updateScores(sessionId) {
		...
		if (metrics.commands.total > 0) {
			...
			const redundancyPenalty = Math.min(
				0.2,
				metrics.commands.redundant * 0.05,
			);
			metrics.scores.commandAccuracy = Math.max(
				0,
				(accuracyRate - redundancyPenalty) * 100,
			);
		}
		...
		metrics.scores.resourceManagement =
			(metrics.resources.powerEfficiency + metrics.resources.fuelEfficiency) /
			2;
		...
		metrics.scores.errorAvoidance = Math.max(
			0,
	...
	checkAchievements(sessionId) {
		...
		if (
			metrics.resources.powerEfficiency > 90 &&
			metrics.resources.fuelEfficiency > 90
		) {
			achievements.push({
				id: "resource_master",
		...
		if (
			metrics.timing.averageResponseTime < 10000 &&
			metrics.timing.responseTimes.length >= 5
		) {
			achievements.push({
				id: "quick_responder",
		...
		if (
			...
			metrics.commands.correct / metrics.commands.total > 0.95
		) {
			achievements.push({
	...
	getSummary(sessionId) {
		...
		return {
			...
			commandAccuracy: (
				(metrics.commands.correct / metrics.commands.total) *
				100
			).toFixed(1),
