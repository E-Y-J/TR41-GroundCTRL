...
const DIFFICULTY_CONFIGS = {
	...
	ADVANCED: {
		enabled: true,
		anomalyChance: 0.3, // 30% chance per check
...
class AnomalyInjector {
	...
	startInjection(sessionId, difficulty, simulationEngine) {
		...
		if (!config || !config.enabled) {
			logger.info("Anomaly injection disabled for difficulty level", {
				sessionId,
				difficulty,
			});
			return;
	...
	checkAndInjectAnomaly(sessionId, config, simulationEngine) {
		...
		if (sessionAnomalies.size >= config.maxConcurrentAnomalies) {
			logger.debug("Max concurrent anomalies reached", {
				sessionId,
				count: sessionAnomalies.size,
			});
			return;
		}
		...
		const eligibleAnomalies = this.getEligibleAnomalies(
			config,
			sessionAnomalies,
		);
		...
		const anomaly =
			eligibleAnomalies[Math.floor(Math.random() * eligibleAnomalies.length)];
	...
	injectAnomaly(sessionId, anomalyDef, simulationEngine) {
		...
		if (simulationEngine?.sessionManager?.io) {
			simulationEngine.sessionManager.io
				.to(sessionId)
	...
	resolveAnomaly(sessionId, commandName, simulationEngine) {
		...
		for (const [anomalyId, anomalyState] of sessionAnomalies.entries()) {
			if (
				anomalyState.active &&
				anomalyState.definition.recoverableWithCommand === commandName
			) {
				...
				if (simulationEngine) {
					simulationEngine.removeAnomalyEffects(
						sessionId,
						anomalyState.definition,
					);
				}
	...
	expireAnomaly(sessionId, anomalyId, simulationEngine) {
		...
		if (anomalyState.definition.severity === "critical") {
			this.handleCriticalAnomalyExpiration(
				sessionId,
				anomalyState,
				simulationEngine,
			);
