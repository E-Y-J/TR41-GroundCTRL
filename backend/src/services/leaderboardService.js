...
async function getGlobalLeaderboard(options = {}) {
  ...
  try {
    // Fetch operators
    const operators = await leaderboardRepository.getTopOperators({ limit, period });
...
async function getLeaderboardWithUserRank(userId, options = {}) {
  try {
    ...
    // Get user's rank
    const userRank = await leaderboardRepository.getUserRank(userId, options.period);
    
    // Get nearby operators
    const nearbyOperators = userRank 
      ? await leaderboardRepository.getNearbyOperators(userId, 5, options.period)
      : [];
...
async function getScenarioLeaderboard(scenarioId, options = {}) {
  ...
  try {
    const operators = await leaderboardRepository.getScenarioLeaderboard(scenarioId, { limit });
