...
class OrbitalMechanics {
	constructor() {
		...
		this.sampleTLEs = {
			ISS: {
				line1:
					"1 25544U 98067A   24031.50000000  .00016717  00000-0  10270-3 0  9005",
				line2:
					"2 25544  51.6416 339.8014 0002735  32.4851  14.8192 15.50574920437487",
			},
			LEO_400KM: {
				line1:
					"1 99999U 24001A   24031.50000000  .00000000  00000-0  00000-0 0  9999",
				line2:
					"2 99999  51.6000   0.0000 0001000   0.0000   0.0000 15.48000000    01",
			},
			LEO_600KM: {
				line1:
					"1 99998U 24001B   24031.50000000  .00000000  00000-0  00000-0 0  9998",
				line2:
					"2 99998  97.5000   0.0000 0001000   0.0000   0.0000 14.84000000    01",
			},
		};
	}
	...
	createSatelliteRecord(line1, line2) {
		try {
			...
			logger.error("Failed to create satellite record", {
				error: error.message,
			});
			return null;
		}
	...
	propagate(satrec, date) {
		try {
			...
			if (positionAndVelocity.error) {
				logger.error("SGP4 propagation error", {
	...
	getSunPosition(timestamp) {
		...
		const lambda = L + 1.915 * Math.sin(M_rad) + 0.02 * Math.sin(2 * M_rad);
		...
		// Sun distance (AU)
		const R =
			1.00014 - 0.01671 * Math.cos(M_rad) - 0.00014 * Math.cos(2 * M_rad);
