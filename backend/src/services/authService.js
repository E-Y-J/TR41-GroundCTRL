...
async function syncOAuthProfile(uid, profileData = {}) {
	...
	try {
		...
		logger.info("OAuth user profile created", {
			uid,
			email,
			callSign: finalCallSign,
		});
...
async function refreshAccessToken(refreshToken) {
	...
	const isBlacklisted =
		await tokenBlacklistRepository.isTokenBlacklisted(refreshToken);
	if (isBlacklisted) {
	...
	try {
		...
		if (!adminsQuery.empty) {
			throw new ConflictError(
				"Admin user already exists. Bootstrap is one-time only.",
			);
		}
...
async function resetPassword(token, newPassword) {
	...
	try {
		...
		if (
			!resetTokenDoc ||
			resetTokenDoc.used ||
			resetTokenDoc.expiresAt < new Date()
		) {
			throw new AuthError("Invalid or expired reset token", 401);
		}
