...
function generateCommandId() {
	return `cmd_${Date.now()}_${require("node:crypto").randomBytes(6).toString("hex")}`;
}
...
class CommandQueue {
	...
	enqueueCommand(command, latencySeconds = null) {
		const actualLatency =
			latencySeconds !== null ? latencySeconds : this.calculateLatency(command);
	...
	processQueue() {
		...
		this.queue.forEach((queuedCmd) => {
			...
			if (
				queuedCmd.status === "uplink_in_progress" &&
				queuedCmd.executeAt <= now
			) {
				...
				try {
					...
					const executionDuration = this.getExecutionDuration(
						queuedCmd.command,
					);
	...
	start() {
		if (this.interval) {
			logger.warn("Command queue already running", {
				sessionId: this.sessionId,
			});
			return;
		}
