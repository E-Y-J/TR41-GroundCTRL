...
describe("Health - Metrics", () => {
	...
	it("should expose metrics endpoint", async () => {
		const response = await request(app).get("/metrics").expect([200, 404]);
	...
	it("should reset metrics periodically", async () => {
		const response1 = await request(app).get("/metrics").expect([200, 404]);

		// Wait and check again
		await new Promise((resolve) => setTimeout(resolve, 1000));

		const response2 = await request(app).get("/metrics").expect([200, 404]);
