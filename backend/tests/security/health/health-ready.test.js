...
describe("Health - Ready State", () => {
	...
	it("should have health check endpoint", async () => {
		const response = await request(app).get("/health").expect([200, 404]);
	...
	it("should report ready status when dependencies are available", async () => {
		const response = await request(app).get("/health/ready").expect([200, 404]);

		if (response.status === 200) {
			expect(response.body).toHaveProperty("status");
			expect(["ready", "ok", "healthy"]).toContain(
	...
	it("should check Firebase connectivity", async () => {
		const response = await request(app).get("/health").expect([200, 503, 404]);
	...
	it("should handle liveness and readiness probes separately", async () => {
		const liveness = await request(app).get("/health/live").expect([200, 404]);
	...
	it("should not require authentication for health checks", async () => {
		const response = await request(app).get("/health").expect([200, 404, 503]);
	...
	it("should respond quickly to health checks", async () => {
		...
		await request(app).get("/health").expect([200, 404, 503]);
