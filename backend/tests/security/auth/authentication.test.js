...
describe("Authentication Security Tests", () => {
	...
	describe("Login Security", () => {
		it("should return generic error for invalid credentials", async () => {
			...
			expect(response.body.payload.error.message).toBe(
				"Invalid email or password",
			);
			expect(response.body.payload.error.message).not.toContain(
				"auth/user-not-found",
			);
		});
	...
	describe("JWT Security", () => {
		it("should validate JWT configuration", async () => {
			...
			expect(process.env.JWT_SECRET).not.toBe(
				"your-super-secret-jwt-key-minimum-64-characters-long-for-production",
			);
		});
	});
