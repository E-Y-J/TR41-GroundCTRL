...
describe("Rate Limit - Global Window Reset", () => {
	...
	it("should reset rate limit counter after window expires", async () => {
		...
		if (blocked) {
			...
			const resetResponse = await request(app).get("/api/v1/satellites");
	...
	it("should track requests per window independently", async () => {
		...
		const response1 = await request(app).get("/api/v1/satellites");
		...
		const response2 = await request(app).get("/api/v1/satellites");
