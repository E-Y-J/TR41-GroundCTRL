...
describe("Rate Limit - Help AI Strict Limit", () => {
	...
	it("should enforce stricter limits on AI help endpoint", async () => {
		const helpAiLimit = parseInt(
			process.env.HELP_AI_RATE_LIMIT_MAX_REQUESTS || "100",
			10,
		);
	...
	it("should track AI endpoint usage separately", async () => {
		...
		const apiResponse = await request(app).get("/api/v1/satellites");

		// Make request to AI endpoint
		const aiResponse = await request(app).post("/api/v1/help/ai").send({
			query: "test question",
			context: "test",
		});
	...
	it("should respect window reset for AI endpoint", async () => {
		const windowMs = parseInt(
			process.env.HELP_AI_RATE_LIMIT_WINDOW_MS || "1000",
			10,
		);

		// Make AI request
		const response1 = await request(app).post("/api/v1/help/ai").send({
			query: "test",
			context: "test",
		});
		...
		const response2 = await request(app).post("/api/v1/help/ai").send({
			query: "another test",
			context: "test",
		});
