...
describe("Input Validation Security Tests", () => {
	...
	describe("Query Parameter Caps", () => {
		it("should handle large query parameters", async () => {
			...
			await request(app).get(`/api/v1/health?test=${longQuery}`).expect(200); // Health endpoint should handle this gracefully
		});
	});
	...
	describe("Sort Whitelist", () => {
		it("should handle sort parameters gracefully", async () => {
			...
			await request(app).get("/api/v1/health?sort=invalidField").expect(200);
		});

		it("should handle valid sort fields", async () => {
			await request(app).get("/api/v1/health?sort=name").expect(200);
		});
	});
