...
describe("Validation - Query Caps", () => {
	...
	it("should limit query string length", async () => {
		...
		const response = await request(app).get(`/api/v1/satellites?${longQuery}`);
	...
	it("should limit individual parameter value length", async () => {
		...
		const response = await request(app).get(
			`/api/v1/satellites?search=${encodeURIComponent(longValue)}`,
		);
	...
	it("should validate page number parameter", async () => {
		const response = await request(app).get(
			"/api/v1/satellites?page=999999999",
		);
	...
	it("should validate limit parameter bounds", async () => {
		const response = await request(app).get("/api/v1/satellites?limit=999999");
	...
	it("should reject negative offset", async () => {
		const response = await request(app).get("/api/v1/satellites?offset=-10");
	...
	it("should validate filter parameter structure", async () => {
		const response = await request(app).get(
			"/api/v1/satellites?filter=invalid_structure",
		);
	...
	it("should prevent deeply nested query parameters", async () => {
		...
		const response = await request(app).get(
			`/api/v1/satellites?${nestedParams}`,
		);
	...
	it("should handle special characters in query values", async () => {
		...
		const response = await request(app).get(
			`/api/v1/satellites?search=${specialChars}`,
		);
	...
	it("should validate date format in query parameters", async () => {
		const response = await request(app).get(
			"/api/v1/satellites?startDate=invalid-date",
		);
	...
	it("should reject query injection attempts", async () => {
		...
		for (const attempt of injectionAttempts) {
			const response = await request(app).get(
				`/api/v1/satellites?search=${encodeURIComponent(attempt)}`,
			);
