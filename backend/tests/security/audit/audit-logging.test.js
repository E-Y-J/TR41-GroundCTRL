...
describe("Audit Logging Security Tests", () => {
	describe("Audit Logging Sanitization", () => {
		it("should not log sensitive data", async () => {
			...
			await request(app).post("/api/v1/auth/login").send(loginData).expect(401); // Wrong password

			// Find the audit log in the calls
			const auditCall = spy.mock.calls.find(
				(call) =>
					call[0]?.includes("[AUDIT]") && call[0].includes("Login attempt"),
			);
	...
	describe("Timestamp in Logs", () => {
		it("should include timestamp in audit logs", async () => {
			...
			const auditCall = spy.mock.calls.find(
				(call) =>
					call[0]?.includes("[AUDIT]") && call[0].includes("Login failed"),
			);
			expect(auditCall).toBeDefined();
		...
		it("should anonymize user data in logs", async () => {
			...
			const auditCall = spy.mock.calls.find(
				(call) =>
					...
					call[0].includes("Login failed") &&
					!call[0].includes("unknown user"),
			);
			expect(auditCall).toBeDefined();
