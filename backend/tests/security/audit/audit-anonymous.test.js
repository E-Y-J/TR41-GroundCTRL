...
describe("Audit - Anonymous", () => {
	...
	it("should log anonymous requests with IP address", async () => {
		const response = await request(app).get("/api/v1/satellites").expect(401);
	...
	it("should assign session ID to anonymous users", async () => {
		...
		const sessionId =
			response.headers["set-cookie"] || response.headers["x-session-id"];
		if (sessionId) {
	...
	it("should log anonymous user actions separately from authenticated", async () => {
		// Anonymous request
		await request(app).get("/api/v1/satellites").expect(401);
