...
describe('Firestore Security Rules - Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    aliceUser = aliceRegister.body.payload.user;
    aliceToken = aliceRegister.body.payload.tokens.accessToken;

    const bobEmail = `bob-${Date.now()}@example.com`;
    const bobRegister = await request(app)
      .post('/api/v1/auth/register')
      .send({ email: bobEmail, password: 'BobPass123!', callSign: 'BOB-01' });

    bobUser = bobRegister.body.payload.user;
    bobToken = bobRegister.body.payload.tokens.accessToken;
  });
  ...
  describe('User Profile Access Control', () => {
    it('allows user to read their own profile via API', async () => {
      ...
      expect(response.body.payload.uid).toBe(aliceUser.uid);
    });
