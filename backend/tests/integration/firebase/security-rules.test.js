...
describe("Firestore Security Rules - Integration Tests", () => {
	...
	beforeAll(async () => {
		if (!isFirebaseActuallyInitialized()) {
			console.log(
				"⚠️  Firebase not initialized - skipping test setup (tests will skip)",
			);
			return;
		}
		...
		aliceToken =
			aliceRegister.body.payload?.tokens?.accessToken ||
			aliceRegister.body.tokens?.accessToken ||
			aliceRegister.body.accessToken;
		...
		_bobToken =
			bobRegister.body.payload?.tokens?.accessToken ||
			bobRegister.body.tokens?.accessToken ||
			bobRegister.body.accessToken;
	});
