...
describe("Satellite API - Integration Tests", () => {
	...
	describe("GET /api/v1/satellites - List Satellites", () => {
		...
		it("should filter satellites by status", async () => {
			...
			const foundSatellite = response.body.payload.data.find(
				(s) => s.id === createdSatelliteId,
			);
	...
	describe("POST /api/v1/commands/execute - Send Command", () => {
		it("should execute a satellite command", async () => {
			...
			const _satelliteId = createResponse.body.payload.data.id;
			...
			if (response.status !== 201) {
				...
				if (response.body.payload?.error?.details) {
					console.log(
						"Validation error details:",
						JSON.stringify(response.body.payload.error.details, null, 2),
					);
				}
			}
			...
			expect(response.body.payload.data.command.commandName).toBe(
				"DEPLOY_SOLAR_ARRAYS",
			);
		});
	});
