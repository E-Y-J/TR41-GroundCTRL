...
describe("AI/NOVA - Integration Tests", () => {
	...
	describe("AI-001: NOVA Help Endpoint - Input Validation", () => {
		...
		it("rejects empty content", async () => {
			...
			expect(response.body.payload.error.details[0].message).toBe(
	...
	describe("AI-002: NOVA Help Endpoint - Fallback Handling", () => {
		it("returns fallback response on timeout", async () => {
			...
			const originalGenerateHelpResponse =
				require("../../../src/services/novaService").generateHelpResponse;
			require("../../../src/services/novaService").generateHelpResponse = jest
				.fn()
				.mockImplementation(
					() =>
						new Promise((resolve) =>
			...
			expect(response.body).toHaveProperty("payload");
			expect(response.body.payload).toHaveProperty("data");
			expect(response.body.payload.data.message).toHaveProperty(
				"is_fallback",
				true,
			);
