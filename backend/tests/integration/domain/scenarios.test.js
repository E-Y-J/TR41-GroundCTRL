...
describe("Scenario Seed Data - Integration Tests", () => {
	...
	beforeAll(async () => {
		...
		for (const scenario of scenarios) {
			...
			console.log(
				`Seeded scenario: ${scenario.code} with ID: ${scenarioRef.id}`,
			);

			// Seed steps for this scenario
			const scenarioSteps = steps.filter(
				(step) => step.scenarioCode === scenario.code,
			);
			console.log(
				`Found ${scenarioSteps.length} steps for scenario ${scenario.code}`,
			);
			for (const step of scenarioSteps) {
				...
				console.log(
					`Seeded step: ${step.data.title} for scenario ${scenarioRef.id}`,
				);
			}
		}
	...
	describe("SEED-001: Satellite Seed Data Validation", () => {
		it("validates training satellite data structure", async () => {
			...
			const trainingSats = satellites.filter((s) =>
				s.code?.startsWith("TRAINING_SAT"),
			);
		...
		it("validates satellite capabilities include expected features", async () => {
			...
			trainingSats.forEach((sat) => {
				...
				expect(allCapabilities).toMatch(
					/(attitude|power|thermal|communication|beacon)/i,
				);
			});
		});
	});
	...
	describe("SEED-002: Scenario Steps and Commands Validation", () => {
		it("validates scenario steps have proper structure", async () => {
			...
			const commissioningScenario = scenarios.find(
				(s) => s.code === "ROOKIE_COMMISSIONING_101",
			);
			...
			if (scenarioData.steps && scenarioData.steps.length > 0) {
				...
				if (firstStep.commands && firstStep.commands.length > 0) {
					firstStep.commands.forEach((command) => {
						...
						expect(["OK", "ERROR", "NO_EFFECT"]).toContain(
