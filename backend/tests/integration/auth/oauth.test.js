...
jest.mock("../../../src/middleware/firebaseAuthMiddleware", () => ({
	firebaseAuthMiddleware: jest.fn((req, _res, next) => {
...
} = require("../../../src/middleware/firebaseAuthMiddleware");
...
describe("OAuth Authentication - Integration Tests", () => {
	...
	describe("AUTH-OAUTH-001: Sync OAuth Profile - New User", () => {
		...
		it("should create new user profile for OAuth authentication", async () => {
			...
			firebaseAuthMiddleware.mockImplementationOnce((req, _res, next) => {
				req.user = {
					uid: "oauth-user-123",
					email: "oauth@example.com",
					emailVerified: true,
			...
			expect(response.body.payload.data.user).toHaveProperty(
				"uid",
				"oauth-user-123",
			);
			expect(response.body.payload.data.user).toHaveProperty(
		...
		beforeEach(() => {
			...
			const userDocMock = {
				get: jest.fn(() =>
					Promise.resolve({
						exists: true,
						data: () => ({ email: "existing-oauth@example.com" }),
					}),
				),
				set: jest.fn(() => Promise.resolve()),
				update: jest.fn(() => Promise.resolve()),
	...
	describe("AUTH-OAUTH-003: Sync OAuth Profile - Missing Authentication", () => {
		it("should reject request without valid Firebase token", async () => {
			...
			firebaseAuthMiddleware.mockImplementationOnce((_req, _res, next) => {
			...
			expect(response.body.payload.error).toHaveProperty(
				"message",
